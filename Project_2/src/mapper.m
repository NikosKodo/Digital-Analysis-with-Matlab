...
% we group the bits into 2 groups. The remainder of the 
%sequence is separately converted into one symbol at
% the end
temp = mod(seq_size, 2);

% the sequence which is dividable by 2
div_seq = bin_seq(1 : (seq_size - temp), :);

% grouping of that sequence
reshaped_sequence = reshape(div_seq, 2, (seq_size - temp) / 2);
